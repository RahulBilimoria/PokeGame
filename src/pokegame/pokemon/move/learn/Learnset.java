...
public class Learnset {
    ...
    public static void init() {
        for (int x = 0; x < 151/*Pokemon.POKEMON_COUNT*/; x++) {
            String file = Utils.loadFileAsString("dat/pokemon/moves/learnable/" + x + ".dat");
            String learnMoves[] = file.split("\\s+");
            LEARN_SET[x] = new Learnset(learnMoves);
        }

    }

    public Move[] getLearnableMove(int pokemonLevel, int levelsGained) {
        int moves = 0;
        int first = -1;
        if (levelsGained != 0) {
            for (int x = 0; x < learnableMoves.length; x++) {
                if (learnableMoves[x].getLevel() > pokemonLevel - levelsGained && learnableMoves[x].getLevel() <= pokemonLevel) {
                    if (first == -1) {
                        first = x;
                    }
                    moves++;
